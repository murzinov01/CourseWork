...
def notification_sender(ch, method, properties, body: bytes):
    ...
    if notification:
        chat_ids = notification.get("chat_ids")
        article = notification.get("article_to_send")
        if chat_ids is not None and article is not None:
...
def main():
    ...
    channel.basic_consume(
        queue=queue_name, on_message_callback=notification_sender, auto_ack=True)
